...
function getDataPasse(url) {
    myRequest=new XMLHttpRequest();
    myRequest.onreadystatechange=getResponsePasse;
        ...
        if (myRequest.readyState === XMLHttpRequest.DONE) {
            switch(myRequest.status) {
                ...
                case 200:
                    ...
                    parcoursJSONPasse(responseData);
...
function parcoursJSONPasse(jsonObj) {
    let phrase = [];
    let premierVerbe;
    let infinitifVerbe;
    for(let i = 0; i< jsonObj.length; i++){
        let name = jsonObj[i]['name'].toLowerCase();        // Renvoie le nom,
        let premPersSing = jsonObj[i]['prem_pers_sing_passe'];    // la première personne du singulier,
        let deuxPersSing = jsonObj[i]['deux_pers_sing_passe'];    // la deuxième,
        let troisPersSing = jsonObj[i]['trois_pers_sing_passe'];  // la troisième,
        let premPersPlur = jsonObj[i]['prem_pers_plur_passe'];    // la première personne du pluriel,
        let deuxPersPlur = jsonObj[i]['deux_pers_plur_passe'];    // la deuxième,
        let troisPersPlur = jsonObj[i]['trois_pers_plur_passe'];  // la troisième,
