...
function getDataFutur(url) {
    myRequest=new XMLHttpRequest();
    myRequest.onreadystatechange=getResponseFutur;
        ...
        if (myRequest.readyState === XMLHttpRequest.DONE) {
            switch(myRequest.status) {
                ...
                case 200:
                    ...
                    parcoursJSONFutur(responseData);
...
function parcoursJSONFutur(jsonObj) {
    let phrase = [];
    let premierVerbe;
    let infinitifVerbe;
    for(let i = 0; i< jsonObj.length; i++){
        let name = jsonObj[i]['name'].toLowerCase();        // Renvoie le nom,
        let premPersSing = jsonObj[i]['prem_pers_sing_futur'];    // la première personne du singulier,
        let deuxPersSing = jsonObj[i]['deux_pers_sing_futur'];    // la deuxième,
        let troisPersSing = jsonObj[i]['trois_pers_sing_futur'];  // la troisième,
        let premPersPlur = jsonObj[i]['prem_pers_plur_futur'];    // la première personne du pluriel,
        let deuxPersPlur = jsonObj[i]['deux_pers_plur_futur'];    // la deuxième,
        let troisPersPlur = jsonObj[i]['trois_pers_plur_futur'];  // la troisième,
